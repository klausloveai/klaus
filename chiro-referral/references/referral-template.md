# Referral email — template & signature

Generic **Referral_Lingtu Law** template (covers Chiro). Subject + body below; fill the
`<...>` slots from the intake + clinic info. Keep wording exact.

## Subject
```
Referral_Lingtu Law_<Client Name>
```
...
Our client(s) sustained injuries in an auto accident. Please contact the patient directly to schedule an initial appointment at your earliest availability.
...
confidentiality footer), so the email **must be sent as HTML**: build an `multipart/mixed`
message = `text/html` body (the content below as `<div>` lines) + the fetched signature HTML +
the attachment. (`build_email.py` is plain-text only — build the HTML MIME inline, see SKILL Step 5.)
