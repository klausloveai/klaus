...
def fill_sum100(cfg, tmpl, out):
    ...
    # SUM-100 line 2 = address; override with sum100_court_line2 when you also want the
    # branch/courthouse named there (CM-010 keeps the bare street/city fields).
    line2 = cfg.get('sum100_court_line2',
                    f"{cfg['courthouse_street']}, {cfg['courthouse_city_zip']}")
    ...
    # SUM-100 "name and address of the court" line 1 = the full COURT name
    # ("Superior Court of California, County of <X>"), NOT the building/branch. Use
    # court_name_sum100 if given; fall back to courthouse_name (LA uses the courthouse).
    court_line1 = cfg.get('court_name_sum100', cfg['courthouse_name'])
    set_text(w, {
        (lambda nm, tu: nm.endswith('FillText25[0]')): defendant,
        (lambda nm, tu: nm.endswith('FillText180[0]')): plaintiff,
        (lambda nm, tu: nm.endswith('FillText3[0]')): court_line1,
...
def fill_cm010(cfg, tmpl, out):
    ...
    firm_phone = cfg.get('firm_phone', '(626) 479-2207')
    firm_fax = cfg.get('firm_fax', '(626) 479-2207')
    set_text(w, {
        (lambda nm, tu: nm.endswith('Phone[0]') or tu.strip() == 'TELEPHONE NUMBER:'): firm_phone,
        (lambda nm, tu: nm.endswith('Fax[0]') or tu.strip() == 'FAX NUMBER:'): firm_fax,
        ...
        (lambda nm, tu: nm.endswith('CrtBranch[0]')): cfg.get('branch_name', cfg['courthouse_name']),
    ...
    # Item 3 punitive — check when the complaint prays for punitive/exemplary
    # damages (Civ. Code s.3294). Config: "punitive": true.
    if cfg.get('punitive'):
        set_button(w, lambda nm, tu: nm.endswith('Ch3[0]'))
    # Item 3 also check "punitive" (Ch3) when the complaint prays for punitive/exemplary
    # damages under Civ. Code §3294 — set config "punitive": true.
    if cfg.get('punitive'):
        set_button(w, lambda nm, tu: nm.endswith('Ch3[0]') or tu.strip() == 'punitive')
...
def main():
    ...
    # County-specific cover-sheet addendum (see references/court-rules.md):
    #   'civ109' → LA CIV 109;  'none' → county needs no addendum (e.g. Ventura for a
    #   PI/dog-bite case — VN278 is an Optional Form that does NOT list PI(23));
    #   other counties add their filler here as they are learned.
    addendum = str(cfg.get('addendum', 'civ109')).lower()
    if addendum == 'civ109':
        fill_civ109(cfg, t['civ109'], os.path.join(outd, '4 - Civil Case Cover Sheet Addendum and Statement of Location.pdf'))
    elif addendum in ('none', '', 'skip'):
        print('no county addendum required for this case type')
    else:
        print('  [warn] unknown addendum type:', addendum, '- prepare it by hand')
    print('DONE ->', outd)
